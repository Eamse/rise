// createAdmin.js
// 🚨 운영 서버에서 딱 한 번만 실행하고 삭제하세요!
// 실행 방법: node createAdmin.js
...
async function main() {
  const admins = [
    {
      id: "master", // 마스터 계정
      pw: process.env.ADMIN_PW_MASTER || null,
      name: "Master Admin",
    },
    {
      id: "admin", // 어드민 계정
  ...
  for (const account of admins) {
    ...
    // 비밀번호 강도 검사 (최소 12자, 영문+숫자+특수기호 포함)
    const pwRegex = /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*]).{12,}$/;
    if (!pwRegex.test(account.pw)) {
      console.error(`❌ [${account.id}] 비밀번호가 너무 약합니다!`);
      console.error(
        `   → 최소 12자 이상, 영문+숫자+특수문자(!@#$%^&*) 포함 필수`,
      );
      continue;
    }

    // bcrypt 라운드 12 (실무 보안 표준 - 10보다 4배 느려서 무차별 대입 공격에 강함)
  ...
  console.log(
    `\n⚠️  완료! 보안을 위해 이 파일(createAdmin.js)을 서버에서 즉시 삭제하세요!`,
  );
  console.log(`   → rm createAdmin.js`);
}
