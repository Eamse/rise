...
import { cancelOrder } from "@/server/orders/services/order.service";
...
export async function POST(
  ...
  try {
    ...
    const data = await cancelOrder(authenticatedUserId, numericOrderId);
    return ok(data);
  } catch (error) {
    return failFromError(error, "주문 취소 중 오류가 발생했습니다.");
  }
}
